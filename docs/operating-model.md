...
- **The skills** → `marketing-feedback` (H2), `onboarding-handoff` (H3), `account-health` (the right-side detector) handing to `churn-save`, `expansion-signal`, and `qbr-prep` (the act-skills), `support-signal` → `product-signal` (H4/H5), `retention-feedback` (the post-sale→product loop).
