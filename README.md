...
| `ops/` | Your plain-text playbooks — `priorities.md`, `daily-log.md`, `pipeline.md` (left side), `customers.md` and `roadmap-signals.md` (right side), `feedback-log.md` (the shared cross-function loop), `chokepoints.md` (the few dependencies most revenue flows through), and `icp.md`. Ships pre-seeded with **fictional** accounts (clearly labelled in each file) so the rituals run day one — replace them with your own. Start here. |
...
| `docs/principles-from-the-field.md` | A third thinking aid: operating principles drawn from running the motion itself (ownership, alignment, daily discipline, decisions, direction & authority, reading the return, dependencies, institutional memory, character & judgment, attention & inputs, conflict & resolution, change, live moments, morale) — each with a worked go-to-market example, plus a map of where they land in the kit |
